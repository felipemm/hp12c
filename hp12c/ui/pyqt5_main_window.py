...
from PyQt5.QtWidgets import QMainWindow, QApplication, QMenuBar, QMenu, QAction, QActionGroup, QMessageBox
from PyQt5.QtGui import QPainter, QFont, QFontDatabase, QFontMetrics, QColor, QPixmap, QIcon, QKeySequence, QClipboard
...
class PyQt5MainWindow(BaseMainWindow):
    ...
    def build(self):
        ...
        if app is None:
            ...
            print("QApplication created in PyQt5MainWindow.build()")
    ...
    def build_menu_bar(self):
        ...
        self._menu_bar = self._frame.menuBar()

        # File menu
        file_menu = self._menu_bar.addMenu(self._language_loader.get_value("FILE_MENU", "File"))

        import_action = QAction(self._language_loader.get_value("FILE_IMPORT", "Import"), self._frame)
        import_action.setShortcut(QKeySequence(f"Ctrl+{self._language_loader.get_shortcut('FILE_IMPORT', 'I').upper()}"))
        import_action.triggered.connect(self._on_file_import)
        file_menu.addAction(import_action)

        export_action = QAction(self._language_loader.get_value("FILE_EXPORT", "Export"), self._frame)
        export_action.setShortcut(QKeySequence(f"Ctrl+{self._language_loader.get_shortcut('FILE_EXPORT', 'E').upper()}"))
        ...
        quit_action.setShortcut(QKeySequence(f"Ctrl+{self._language_loader.get_shortcut('FILE_QUIT', 'Q').upper()}"))
        ...
        copy_action.setShortcut(QKeySequence(f"Ctrl+{self._language_loader.get_shortcut('EDIT_COPY', 'C').upper()}"))
        ...
        paste_action.setShortcut(QKeySequence(f"Ctrl+{self._language_loader.get_shortcut('EDIT_PASTE', 'P').upper()}"))
        ...
        erase_display_action = QAction(self._language_loader.get_value("EDIT_ERASE_DSP", "Display"), self._frame)
        erase_display_action.triggered.connect(self._on_edit_erase_display)
        erase_menu.addAction(erase_display_action)

        erase_stack_action = QAction(self._language_loader.get_value("EDIT_ERASE_STK", "Stack Registers"), self._frame)
        erase_stack_action.triggered.connect(self._on_edit_erase_stack)
        erase_menu.addAction(erase_stack_action)

        erase_finance_action = QAction(self._language_loader.get_value("EDIT_ERASE_FIN", "Finance Registers"), self._frame)
        erase_finance_action.triggered.connect(self._on_edit_erase_finance)
        erase_menu.addAction(erase_finance_action)

        erase_statistic_action = QAction(self._language_loader.get_value("EDIT_ERASE_STA", "Statistic Registers"), self._frame)
        erase_statistic_action.triggered.connect(self._on_edit_erase_statistic)
        erase_menu.addAction(erase_statistic_action)

        erase_all_action = QAction(self._language_loader.get_value("EDIT_ERASE_REG", "All Registers"), self._frame)
        erase_all_action.triggered.connect(self._on_edit_erase_all)
        erase_menu.addAction(erase_all_action)

        erase_program_action = QAction(self._language_loader.get_value("EDIT_ERASE_PRG", "Program Steps"), self._frame)
        ...
        # Skin submenu
        skin_menu = view_menu.addMenu(self._language_loader.get_value("VIEW_SKIN", "Calculator skin"))
        ...
        # Options menu
        options_menu = self._menu_bar.addMenu(self._language_loader.get_value("OPTIONS_MENU", "Options"))

        # Number format submenu
        num_format_menu = options_menu.addMenu(self._language_loader.get_value("OPTIONS_NUMBER_FORMAT", "Number Format"))
        num_format_group = QActionGroup(self._frame)

        dot_action = QAction(self._language_loader.get_value("OPTIONS_NUMBER_FORMAT_DOT", "Dot (.)"), self._frame)
        ...
        comma_action = QAction(self._language_loader.get_value("OPTIONS_NUMBER_FORMAT_COMMA", "Comma (,)"), self._frame)
        comma_action.setCheckable(True)
        comma_action.setChecked(self._cfg and self._cfg.get_com() == 1)
        ...
        # Date format submenu
        date_format_menu = options_menu.addMenu(self._language_loader.get_value("OPTIONS_DATE_FORMAT", "Date Format"))
        date_format_group = QActionGroup(self._frame)

        mdy_action = QAction(self._language_loader.get_value("OPTIONS_DATE_FORMAT_MONTH", "MM.DDYYYY"), self._frame)
        ...
        dmy_action = QAction(self._language_loader.get_value("OPTIONS_DATE_FORMAT_DAY", "DD.MMYYYY"), self._frame)
        dmy_action.setCheckable(True)
        dmy_action.setChecked(self._cfg and self._cfg.get_dmy() == 1)
        ...
        payment_menu = options_menu.addMenu(self._language_loader.get_value("OPTIONS_PAYMENT_MODE", "Payment mode"))
        payment_group = QActionGroup(self._frame)

        begin_action = QAction(self._language_loader.get_value("OPTIONS_PAYMENT_MODE_BEGIN", "Begin"), self._frame)
        begin_action.setCheckable(True)
        begin_action.setChecked(self._cfg and self._cfg.get_beg() == 1)
        ...
        end_action = QAction(self._language_loader.get_value("OPTIONS_PAYMENT_MODE_END", "End"), self._frame)
        ...
        registers_action = QAction(self._language_loader.get_value("TOOLS_REGISTERS_VIEW", "Registers view"), self._frame)
        registers_action.triggered.connect(self._on_tools_registers_view)
        tools_menu.addAction(registers_action)

        history_action = QAction(self._language_loader.get_value("TOOLS_HISTORY", "Instructions history"), self._frame)
        ...
        author_action = QAction(self._language_loader.get_value("ABOUT_AUTHOR", "Author"), self._frame)
        author_action.triggered.connect(self._on_about_author)
        about_menu.addAction(author_action)

        contributors_action = QAction(self._language_loader.get_value("ABOUT_CONTRIBUTORS", "Contributors"), self._frame)
        contributors_action.triggered.connect(self._on_about_contributors)
        about_menu.addAction(contributors_action)

        software_action = QAction(self._language_loader.get_value("ABOUT_SOFTWARE", "This Software"), self._frame)
    ...
    def _on_options_number_format(self):
        ...
        action = self.sender()
        if action and self._cfg and self._controller:
    ...
    def _on_tools_history(self):
        ...
        print("Tools > Instructions history (not implemented)")
